...
export async function exportToPDF(requirement: Requirement): Promise<void> {
  ...
  // 辅助函数：添加图片
  const addImage = async (imageUrl: string, maxWidth: number = contentWidth, maxHeight: number = 100) => {
    try {
      // 如果是 dataURL
      if (imageUrl.startsWith('data:')) {
        ...
        doc.addImage(imageUrl, 'PNG', margin, currentY, width, height);
        currentY += height + 10;
      }
    } catch (e) {
  ...
  // 标题页
  addText(requirement.title, 24, true, '#1a1a1a');
  currentY += 10;
  
  // 元信息
  addText(`状态: ${getStatusLabel(requirement.status)}`, 11);
  addText(`优先级: ${requirement.priority}`, 11);
  addText(`创建时间: ${new Date(requirement.createdAt).toLocaleString('zh-CN')}`, 11);
  currentY += 10;
  ...
  // 用户描述
  addText('需求描述', 16, true);
  addText(requirement.userDescription || '未提供', 11);
  currentY += 10;
  
  // 截图部分
  if (requirement.screenshots.length > 0) {
    addText(`截图 (${requirement.screenshots.length}张)`, 16, true);
    
    for (let i = 0; i < requirement.screenshots.length; i++) {
      const screenshot = requirement.screenshots[i];
      ...
      addText(`截图 ${i + 1}: ${screenshot.title || '未命名'}`, 12, true);
      addText(`页面: ${screenshot.pageTitle || '未知'}`, 10);
      addText(`URL: ${screenshot.pageUrl || screenshot.url || '未知'}`, 9);
      
      if (screenshot.annotations.length > 0) {
        addText(`标注: ${screenshot.annotations.length} 个`, 10);
      }
      
      if (screenshot.imageUrl) {
        await addImage(screenshot.imageUrl, contentWidth * 0.8, 80);
      }
  ...
  if (requirement.aiGeneratedContent) {
    addNewPage();
    addText('AI 生成内容', 18, true);
    currentY += 5;
    
    const suggestions = requirement.aiGeneratedContent.designSuggestions;
    
    // 布局风格
    if (suggestions?.layout) {
      addText('布局风格', 14, true);
      addText(`风格: ${suggestions.layout.style || '未指定'}`, 11);
      addText(`描述: ${suggestions.layout.description || '未提供'}`, 11);
      currentY += 5;
    }
    
    // 配色方案
    if (suggestions?.styleGuide?.colors?.length) {
      addText('配色方案', 14, true);
      suggestions.styleGuide.colors.forEach(color => {
        addText(`• ${color}`, 11);
      });
      currentY += 5;
    }
    ...
    // 组件建议
    if (suggestions?.components?.length) {
      addText('组件建议', 14, true);
      suggestions.components.forEach(comp => {
        addText(`• ${comp.name} (${comp.type})`, 11, true);
    ...
    // 生成 Prompt
    if (requirement.aiGeneratedContent.generatedPrompt) {
      addNewPage();
      addText('生成 Prompt', 14, true);
      addText(requirement.aiGeneratedContent.generatedPrompt, 9);
    }
  }
  
  // 效果图
  if (requirement.mockupDesigns?.length) {
    addNewPage();
    addText(`效果图 (${requirement.mockupDesigns.length}张)`, 18, true);
    ...
    if (selectedMockup) {
      addText('已选中的方案', 14, true);
      addText(`方案: ${selectedMockup.variant}`, 11);
      addText(`风格: ${selectedMockup.style}`, 11);
      addText(`生成批次: ${selectedMockup.generationBatch}`, 11);
      
      if (selectedMockup.imageUrl) {
        await addImage(selectedMockup.imageUrl, contentWidth * 0.9, 100);
      }
    }
    
    // 其他效果图
    const otherMockups = requirement.mockupDesigns.filter(
      m => m.id !== requirement.selectedMockupId
    );
    
    for (const mockup of otherMockups) {
      if (currentY > pageHeight - 100) {
        addNewPage();
      }
      
      addText(`方案 ${mockup.variant} (批次 ${mockup.generationBatch})`, 12, true);
      
      if (mockup.imageUrl) {
        await addImage(mockup.imageUrl, contentWidth * 0.8, 80);
      }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `由 雕花 导出 | ${new Date().toLocaleString('zh-CN')} | 第 ${i} / ${totalPages} 页`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    );
  ...
  // 下载 PDF
  doc.save(`${requirement.title}_PRD.pdf`);
}
