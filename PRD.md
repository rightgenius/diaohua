...
| 截图功能 | ⚠️ 有问题 | xcap 全屏截图，但存在黑屏问题 |
| 截图编辑器 | ✅ 已完成 | 矩形、圆形、箭头、文字、马赛克、裁剪 |
| 标注编辑器 | ✅ 已完成 | Fabric.js 基础标注 |
| AI PRD生成 | 📝 待集成 | UI已准备，待接入 Gemini API |
| AI效果图生成 | 📝 待集成 | UI已准备，待接入 Gemini Imagen |
| 协作与分享 | ✅ 已完成 | 分享链接、评论系统、通知中心 |
| 导出功能 | ✅ 已完成 | JSON、Markdown、PDF |
| 七牛云存储 | ✅ 已完成 | 配置管理、上传功能 |
...
**截图能力**：
- 使用 `xcap` crate 实现跨平台全屏截图（macOS/Windows/Linux）
- 全屏截图编辑器，支持裁剪和标注
- 截图自动添加到需求截图列表
- ⚠️ **已知问题**：截图功能存在黑屏问题，需要修复
...
### 3.5 AI 效果图生成模块（Gemini Imagen）
**功能描述**：基于AI生成的设计参考，自动调用 **Google Gemini API（Imagen 3）** 生成可视化效果图
